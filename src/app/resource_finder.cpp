...
void ResourceFinder::includeDataDir(const char* filename)
{
  ...
  // $HOME/.config/Tekisasu-Graphics/filename
  ...
  // $BINDIR/../share/Tekisasu-Graphics/data/filename (installed in /usr/ or /usr/local/)
...
void ResourceFinder::includeHomeDir(const char* filename)
{
  ...
  // %AppData%/Tekisasu-Graphics/filename
...
void ResourceFinder::includeUserDir(const char* filename)
{
  ...
  if (const char* env = std::getenv("ASEPRITE_USER_FOLDER")) {
    ...
    // $HOME/Library/Application Support/Tekisasu-Graphics/filename
    addPath(
      base::join_path(
