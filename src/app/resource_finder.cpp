...
void ResourceFinder::includeDataDir(const char* filename)
{
  ...
  std::snprintf(buf, sizeof(buf), "data/%s", filename);
  includeUserDir(buf); // $HOME/Library/Application Support/Tekisasugraphics/data/filename
...
void ResourceFinder::includeHomeDir(const char* filename)
{
  ...
  // %AppData%/Tekisasugraphics/filename
  wchar_t* env = _wgetenv(L"AppData");
  if (env) {
    std::string path = base::join_path(base::to_utf8(env), "Tekisasugraphics");
...
void ResourceFinder::includeUserDir(const char* filename)
{
  ...
  if (const char* env = std::getenv("ASEPRITE_USER_FOLDER")) {
    ...
    // $HOME/Library/Application Support/Tekisasugraphics/filename
    addPath(
      base::join_path(
